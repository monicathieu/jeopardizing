...
### Better experts tend to be more confident

This effect does not necessarily imply that better experts are _overconfident._ Rather, it is consistent with better experts tending to have higher latent knowledge for each question, and thus being likely to be more confident overall, even when the answer turns out to be wrong.

```{r}
j_recall %>% 
  group_by(subj_num, j_score) %>% 
  summarize(conf_q500 = median(resp_conf, na.rm = TRUE),
            conf_q250 = quantile(resp_conf, .25, na.rm = TRUE),
            conf_q750 = quantile(resp_conf, .75, na.rm = TRUE),
            conf_q025 = quantile(resp_conf, .025, na.rm = TRUE),
            conf_q975 = quantile(resp_conf, .975, na.rm = TRUE)) %>% 
  ggplot(aes(x = subj_num, y = conf_q500, color = j_score)) +
  # geom_errorbar(aes(ymin = conf_q025, ymax = conf_q975), width = 0, size = 0.5) +
  geom_errorbar(aes(ymin = conf_q250, ymax = conf_q750), width = 0, size = 1) +
  geom_point(size = 3) +
  coord_flip() +
  scale_color_viridis_c() +
  theme_bw() +
...
### Better experts tend to respond faster

Again, this is consistent with an across-participants tendency to respond faster for subsequently correct answers. Better experts who are giving more correct answers are thus more likely to have shorter RTs.

```{r}
j_recall %>% 
  group_by(subj_num, j_score) %>% 
  summarize(rt_q500 = median(rt_end_recall, na.rm = TRUE),
            rt_q250 = quantile(rt_end_recall, .25, na.rm = TRUE),
            rt_q750 = quantile(rt_end_recall, .75, na.rm = TRUE),
            rt_q025 = quantile(rt_end_recall, .025, na.rm = TRUE),
            rt_q975 = quantile(rt_end_recall, .975, na.rm = TRUE),
            rt_mean = mean(rt_end_recall, na.rm = TRUE),
            rt_sd = sd(rt_end_recall, na.rm = TRUE)) %>% 
  ggplot(aes(x = subj_num, y = rt_mean, color = j_score)) +
  geom_errorbar(aes(ymin = rt_mean - rt_sd/2, ymax = rt_mean + rt_sd/2), width = 0, size = 1) +
  geom_point(size = 3) +
  coord_flip() +
  scale_color_viridis_c() +
  theme_bw() +
...
```{r}
encoding %>% 
  ggplot(aes(x = j_score, y = resp, color = category, fill = category)) +
...
```{r}
encoding %>% 
  mutate(already_knew = fct_relevel(already_knew, "none", "some")) %>% 
...
retrieval %>% 
  ...
  geom_jitter(alpha = 0.3, height = 0.05) + 
  geom_smooth(aes(group = subj_num), method = "glm", method.args = list(family = "binomial"), alpha = 0.02) + 
  scale_color_viridis_c() +
  scale_fill_viridis_c() +
  # facet_grid(~already_knew) +
...
retrieval %>% 
  ...
  mutate(acc_recall = as.numeric(acc_recall > 0),
         ...
         j_score = j_score - 0.5) %>% 
  lme4::glmer(acc_recall ~ resp_pic * j_score + (1 | subj_num), data = ., family = "binomial") %>% 
  summary()
```
...
retrieval %>% 
  filter(already_knew == "none") %>% 
  mutate(acc_recall = if_else(acc_recall == 0.5, 1, acc_recall),
         j_score_binary = j_score > 0.72) %>%
  ggplot(aes(x = resp_source, y = acc_recall, color = cut_number(j_score, 4), fill = cut_number(j_score, 4))) + 
  geom_jitter(alpha = 0.3, height = 0.05) + 
  geom_smooth( method = "glm", method.args = list(family = "binomial"), alpha = 0.2) + 
  labs(x = "Source recognition accuracy", y = "Fact recall accuracy (generous)") +
  theme_bw()
...
retrieval %>% 
  ...
  mutate(acc_recall = as.numeric(acc_recall > 0),
         ...
         j_score = j_score - 0.5) %>% 
  lme4::glmer(acc_recall ~ resp_source * j_score + (1 | subj_num), data = ., family = "binomial") %>% 
  summary()
```
...
retrieval %>% 
  filter(already_knew == "none") %>% 
  ggplot(aes(x = resp_source, y = resp_pic, color = cut_number(j_score, 5), fill = cut_number(j_score, 5))) + 
...
retrieval %>% 
  filter(already_knew == "none") %>% 
  mutate(acc_recall = if_else(acc_recall == 0.5, 1, acc_recall)) %>%
  ggplot(aes(x = interest, y = acc_recall, color = cut_number(j_score, 4), fill = cut_number(j_score, 4))) + 
  geom_jitter(alpha = 0.3, height = 0.05) + 
  geom_smooth( method = "glm", method.args = list(family = "binomial"), alpha = 0.2) + 
  labs(x = "Interest in fact at encoding", y = "Fact recall accuracy (generous)") +
  theme_bw()
...
retrieval %>% 
  # filter(already_knew == "none") %>% 
  ggplot(aes(x = interest, y = resp_pic, color = cut_number(j_score, 4), fill = cut_number(j_score, 4))) + 
  geom_point(alpha = 0.1) + 
  geom_smooth(method = "lm", alpha = 0.2) + 
...
retrieval %>% 
  filter(already_knew == "none") %>% 
  ggplot(aes(x = interest, y = resp_source, color = cut_number(j_score, 4), fill = cut_number(j_score, 4))) + 
  geom_point(alpha = 0.1) + 
  geom_smooth(method = "lm", alpha = 0.2) + 

...
j_recall %>% 
  ...
  mutate(acc_recall = as.integer(acc_recall),
         ...
         j_score = j_score - 0.5,
         is_male = gender == "male") %>% 
  lme4::lmer(resp_conf ~ acc_recall * j_score + is_male + (1 + acc_recall | subj_num), data = .) %>% 
  summary()
```
...
j_recall %>% 
  left_join(q_google_demos, by = "subj_num") %>% 
  ggplot(aes(x = acc_recall, y = resp_conf, color = gender)) + 
  geom_jitter(alpha = 0.3, width = 0.05) + 
  geom_smooth(aes(group = subj_num), method = "lm", formula = y ~ x, se = FALSE, size = 0.5) + 
  labs(x = "Recall accuracy (strict)", y = "Recall confidence", color = "Expertise score", fill = "Expertise score") +
  theme_bw()
```
...
retrieval %>% 
  ...
  mutate(acc_recall = as.numeric(acc_recall > 0),
         ...
         j_score = j_score - 0.5) %>% 
  lme4::glmer(acc_recall ~ interest * j_score + (1 | subj_num), data = ., family = "binomial") %>% 
  summary()
```

...
```{r}
retrieval <- r_facts %>%
  select(subj_num, group, group_trial_num, retrieval_block, ep_is_same, sem_is_same, retrieval_trial_num, category, acc_recall, rt_recall = rt) %>% 
  left_join(r_pics %>%
              mutate(resp = resp - 50,
...
retrieval %>% 
  ...
  geom_line() +
  geom_point(aes(color = retrieval_block)) +
...
```{r}
retrieval %>% 
  group_by(subj_num, ep_is_same, sem_is_same) %>% 

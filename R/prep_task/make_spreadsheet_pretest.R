...
make_spreadsheet_pretest <- function (master_spreadsheet_path, stimlist_folder) {
  pretest <- master_spreadsheet_path %>%
    read_csv() %>% 
  ...
  pretest %$% 
    walk2(objects, category,
          ~write_csv(.x, here::here(stimlist_folder, .y), na = ""))
  
  # return PATHS TO WRITTEN OUT SPREADSHEETS
  return (here::here(stimlist_folder, pretest$category))
}

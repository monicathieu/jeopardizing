...
score_retrieval_facts_firstpass <- function (retrieval_data_unscored) {
  out <- retrieval_data_unscored %>% 
...
score_retrieval_facts_byhand <- function (retrieval_data_prescored, handscoring_data, interactive = FALSE, reset_scores = FALSE) {
  byhand <- handscoring_data %>% 
    distinct()
  
  # This leaves non-hand-scored entries as NA
  scored <- retrieval_data_prescored %>% 
    left_join(byhand)
  
  if (interactive) {
    if (reset_scores) scored$acc_recall_byhand <- NA_real_
...
score_retrieval_facts_lastpass <- function (retrieval_data_handscored) {
  out <- retrieval_data_handscored %>%
